...
export const LOOP_JOBS: LoopJobDef[] = [
  ...
  },
  {
    id: "learn-match",
    name: "Match 가중치 학습",
    description: "outcome_events(watch/match/unlock) → brain_weights 소폭 보정·활성화.",
    cadence: "daily",
    pipeline: "learn-match",
    domains: ["deal_price"],
    targetTables: ["brain_weights"],
    defaultEnabled: true,
    changeDetect: "always",
    requiresEnv: ["SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"],
  },
  {
    id: "timing-eval",
    name: "Timing 사후 평가",
    description: "과거 timing_verdict vs 이후 중앙값 변화 → timing_evals correct/miss.",
    cadence: "daily",
    pipeline: "timing-eval",
    domains: ["deal_price"],
    targetTables: ["timing_evals"],
    defaultEnabled: true,
    changeDetect: "always",
    requiresEnv: ["SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"],
  },
  {
    id: "signal-alerts",
    name: "BUY 전환 알림 큐",
    description: "wait→buy 전환 + watchlist/digest → signal_alert_queue.",
    cadence: "daily",
    pipeline: "signal-alerts",
    domains: ["deal_price"],
    targetTables: ["signal_alert_queue"],
    defaultEnabled: true,
    changeDetect: "fingerprint",
    requiresEnv: ["SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"],
  },
  {
    id: "send-alerts",
    name: "알림 이메일 발송",
    description: "signal_alert_queue pending → Resend (키 없으면 dry preview).",
    cadence: "daily",
    pipeline: "send-alerts",
    domains: ["deal_price"],
    targetTables: ["signal_alert_queue"],
    defaultEnabled: true,
    changeDetect: "always",
  ...
  },
  {
    ...
    name: "car_profiles stub 부트스트랩",
    description: "트림만 있고 프로필 없는 차종에 발행 stub 생성 (표본 0).",
    cadence: "daily",
    pipeline: "profile-bootstrap",
    domains: ["catalog"],
    targetTables: ["car_profiles"],
    defaultEnabled: true,
    changeDetect: "fingerprint",
    requiresEnv: ["SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"],
  },
